...
<div class="topnav">
 		...
 		<a class = "active" href=managepat.php>Manage Patients</a>
		...
		echo "<table border='1'>";
		echo "<tr><th>PatientID</th><th>Name</th><th>Email</th><th>Password</th><th>Phone</th><th>City</th><th>Insured</th><th>Insurance Number</th><tr>";
		while($row = mysqli_fetch_assoc($result))
		{
		  
		   echo "<tr><td>{$row['pat_id']}</td><td>{$row['pat_name']}</td><td>{$row['pat_email']}|</td><td>{$row['pat_password']}</td><td>{$row['pat_phone']}</td><td>{$row['pat_city']}</td><td>{$row['pat_insured']}</td><td>{$row['pat_insurance_num']}</td><tr>";
		...
		?>
		<br><h3>VIEW ALL PATIENT CONDITIONS</h3>
		<form method ="post">
		<button type="submit" name="viewCona" value="View Conditions" class="btn btn-primary"> View Conditions</button>
		</form>
		<?php
		include "db_conn.php";
		//session_start();
		if(isset($_POST['viewCona'])){
            function validate($entry){
                $entry=trim($entry);
                $entry=stripslashes($entry);
                $entry=htmlspecialchars($entry);
		...
		//$sid=$_SESSION['ssid'];
		$query = "SELECT * FROM patient_conditions";
		$result= mysqli_query($conn, $query);
		if(mysqli_num_rows($result)== 0){  
			echo "<script>
	...
	<br><h3>VIEW PATIENT CONDITIONS</h3>
		...
		<br><h3>DELETE PATIENTS</h3>
		<form method ="post">
		<label>Enter Patient ID:</label>
		<input type="text" name="pat_idd" placeholder="Patient ID"/> <br>
		<button type="submit" name="deletePatient" value="Delete Patient" class="btn btn-primary"> Delete Patient </button>
		</form>
	</div>
	<?php
		include "db_conn.php";
		if(isset($_POST['pat_idd'])){
			function validate($entry){
			$entry=trim($entry);
			$entry=stripslashes($entry);
			$entry=htmlspecialchars($entry);
		...
		$sqle="SELECT * FROM patient WHERE pat_id='$id'";
		...
		$sql2="SELECT * FROM patient WHERE pat_id='$id'";
